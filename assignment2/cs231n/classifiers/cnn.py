...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ds, dW3, db3 = affine_backward(ds, cache3)
    ds, dW2, db2 = affine_relu_backward(ds, cache2)
    dx, dW1, db1 = conv_relu_pool_backward(ds, cache1)
    loss += 0.5 * self.reg * sum([np.sum(w*w) for w in [W1, W2, W3, b1, b2, b3]])
    grads = {
